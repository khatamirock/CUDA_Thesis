 ...
 *
 */
 __global__ void findMin(uchar* result, uchar *a, uchar *b, uchar *c) {
    
    uchar *temp_min ;
    cudaMalloc(&temp_min,32);
    temp_min=a;
     if (b < temp_min) {
        temp_min = b;
    }
 ...
 __global__ void findMax(uchar* result, uchar *a, uchar *b, uchar *c) {
...
__global__ void convert(uchar *d_r, uchar *d_g, uchar *d_b, uchar *d_gray)
{
    ...
    int Currblock=blockIdx.x+gridDim.x*blockIdx.y+gridDim.x*gridDim.y*blockIdx.z;
    int thread= Currblock*(blockDim.x*blockDim.y) 
        +blockDim.x*threadIdx.y+threadIdx.x ;//+blockDim.x*blockDim.y*blockIdx.z;
    


    if(thread<=1000*1333)
      d_gray[thread]=(d_r[thread]+d_g[thread]+d_b[thread])/3;
...
__host__ void executeKernel(uchar *d_r, uchar *d_g, uchar *d_b, uchar *d_gray, int rows, int columns, int threadsPerBlock)
{
    ...
    int gridCols = min(columns/(threadsPerBlock*4),2);
    dim3 grid( gridCols,rows, 3);
    dim3 block( threadsPerBlock, blockZSize);
